    ...
    try:
        ...
        if mode_refill:
            products = conn.execute("""
                SELECT item_name, consumption_unit, current_quantity, category, usage_freq_qty, usage_freq_days 
                FROM products 
            ...
            for p in products:
                ...
                if current < total_needed:
                    ...
                    conn.execute("INSERT INTO shopping_list_items (list_id, item_name, consumption_unit, suggested_quantity, adjusted_quantity, current_stock, category, diet_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
                                (list_id, p['item_name'], p['consumption_unit'], total_needed, to_buy, current, p['category'], 'Custom'))
                    count += 1
            ...
            for item in catalog:
                ...
                conn.execute("INSERT INTO shopping_list_items (list_id, item_name, consumption_unit, suggested_quantity, adjusted_quantity, current_stock, category, diet_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
                            (list_id, item['item_name'], item['consumption_unit'], needed, to_buy, curr, item['category'], item['diet_type']))
                count += 1
...
def add_item_to_list(list_id):
    ...
    # Store usage data if provided in manual add
    usage_qty = safe_float(data.get('usageQty', 1))
    usage_period = safe_float(data.get('usagePeriod', 7))
...
# --- CONFIRM LIST (WITH NOTIFICATION) ---
